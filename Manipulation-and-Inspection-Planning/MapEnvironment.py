...
class MapEnvironment(object):
    ...
    def interpolate_plan(self, plan_configs):
        '''
        Interpolate plan of configurations - add steps between each two configs to make visualization smoother.
    ...
    def visualize_plan(self, plan, cost, run_time,ext_mode,goal_prob):
        '''
        ...
        # store gif
        directory = "results_" + str(self.task)

        # Create the directory if it doesn't exist
        if not os.path.exists(directory):
            os.makedirs(directory)
        cost_str = "{:.2f}".format(cost)
        run_time_str = "{:.2f}".format(run_time)
        plan_time = datetime.now().strftime("%d-%m-%Y_%H-%M-%S")
        file_path = os.path.join(directory, f'{ext_mode}_{goal_prob}_cost {cost_str} time {run_time_str}.gif')
        imageio.mimsave(file_path, plan_images, 'GIF', duration=0.05)
